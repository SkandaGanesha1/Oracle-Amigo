...
export class HeartbeatService {
  ...
  async pulse(): Promise<unknown> {
    const result = await withRecoveredDeviceToken(this.store, this.profileId, async (identity) =>
      new PresenceClient(new ControlPlaneClient(identity.controlPlaneUrl)).heartbeat({
